...
###########
# GUI Creation
from tkinter import Button, Canvas, Label, Menu, Scale, Tk  # Visual components
from tkinter import BOTH, CENTER, E, LEFT, N, RIGHT, S, W, YES  # Variables
from tkinter import TclError  # Error detection
...
# Music
# Stop pygame's welcome message popping up in console
...
class musicApp(Tk):
    def __init__(self):
        ...
        # If it is the user's first time running the code or preferences is missing
    ...
    def setDefaultTheme(self):
        # Puts import darkdetect here as it is only used if needed
        # isDark() checks if the user's device has their default theme as dark or light
    ...
    def switchPage(self, frameClass):
...
class StartPage(ttk.Frame):
    def __init__(self, master):
        ...
        self.intervalsButton = Button(
            self,
            text="Intervals",
            command=lambda: master.switchPage(IntervalsTutorialPage)
        )
        self.intervalsButton.grid(row=2, column=1, sticky=N + S + E + W)

        self.sightReadingButton = Button(
            self,
            text="\n\n\n  Sight Reading \n\n\n",
        ...
        self.terminologyButton = Button(
            self,
            text="  Terminology   ",
            command=lambda: master.switchPage(TerminologyFlashCardsPage)
        )
        self.terminologyButton.grid(row=3, column=1, sticky=N + S + E + W)

        self.instrumentPracticeButton = Button(
            self,
            text="\n\nInstrument\nPractice\n\n",
...
class IntervalsTutorialPage(ttk.Frame):
    def __init__(self, master):
        ...
        # Image credit: Hyacinth, CC BY-SA 3.0, via Wikimedia Commons+
        intervalExampleImage = Image.open("images/exampleIntervals.png")
        ...
        informationLabel = ttk.Label(
            self,
            text='''
            Intervals is the distance between two notes. Above is a picture showing what all the
            intervals will look like at the tonic of middle C. The way to tell what the interval is
            by counting how many lines and gaps there are between the notes starting on the first
            note. For example, a perfect fourth would have 4 lines and gaps in between it.\n
            ''',
        ...
        harmonicIntervalTestButton = Button(
            self,
            text="Harmonic Interval Test",
            command=lambda: master.switchPage(HarmonicIntervalsTest),
            height=8
        )
        harmonicIntervalTestButton.pack(side=LEFT, fill='x', padx=128)

        # Button to the melodic interval test
        melodicIntervalTestButton = Button(
            self,
            text="Melodic Interval Test",
            command=lambda: master.switchPage(MelodicIntervalsTest),
            height=8
        )
...
class HarmonicIntervalsTest(ttk.Frame):
    def __init__(self, master):
        ...
        # Add score counter
        self.score = 0
        self.scoreCounter = ttk.Label(
            self,
            text=f"Score: {str(self.score)}/{str(self.questionCounter)}",
        ...
        intervalImage = Image.open(
            f"images/intervals/harmonic/{self.fileValues[0]}/{self.fileValues[1]}/{self.fileValues[2]}"
        )
        ...
        ttk.Label(
            ...
            font=("TkDefaultTheme", 32)
        ).pack()

        # Create buttons of the intervals being tested
        self.p5Button = Button(
            self,
            text="Perfect\nFifth",
            command=lambda: self.buttonClicked(master, "P5"),
            height=8,
            width=8
        )
        self.p5Button.pack(side=LEFT, fill='x', padx=128)

        self.octaveButton = Button(
            self,
            text="Octave",
            command=lambda: self.buttonClicked(master, "Octave"),
            height=8,
            width=8
        )
...
class MelodicIntervalsTest(ttk.Frame):
    def __init__(self, master):
        ...
        # Create the image of a random interval
        intervalImage = Image.open(
            f"images/intervals/melodic/{self.fileValues[0]}/{self.fileValues[1]}/{self.fileValues[2]}"
        )
        self.intervalImg = ImageTk.PhotoImage(intervalImage)
        self.harmonicIntervalImageLabel = Label(self, image=self.intervalImg)
        self.harmonicIntervalImageLabel.pack()
...
class SightReadingTutorialPage(ttk.Frame):
    def __init__(self, master):
        ...
        # Image made by Matthew McDermott using Musescore
        sightReadingExampleImage = Image.open("images/exampleSightReading.png")
        ...
        ttk.Label(
            ...
            text='''
             ...
             A to G alphabetically. When you reach G, you go back to A for the next note.
            ''',
            font=("TkDefaultTheme", 20),
            justify=CENTER
        ).pack()
        ...
        Button(
            self,
            text="Sight Reading Test",
            command=lambda: master.switchPage(SightReadingTest),
            height=8
        ).pack()
...
class SightReadingTest(ttk.Frame):
    def __init__(self, master):
        ...
        sightReadingImage = Image.open(
            f"images/sightReading/{self.fileValues[0]}/{self.fileValues[1]}/{self.fileValues[2]}"
        )
        ...
        self.aFlatButton = Button(
            self,
            text="Ab",
            command=lambda: self.buttonClicked(master, "Ab"),
            height=5,
            width=5
        )
        ...
        self.aButton = Button(
            self,
            text="A",
            command=lambda: self.buttonClicked(master, "A"),
            height=5,
            width=5
        )
        self.aButton.pack(side=LEFT)

        self.bFlatButton = Button(
            self,
            text="Bb",
            command=lambda: self.buttonClicked(master, "Bb"),
            height=5,
            width=5
        )
        ...
        self.bButton = Button(
            self,
            text="B",
            command=lambda: self.buttonClicked(master, "B"),
            height=5,
            width=5
        )
        ...
        self.cButton = Button(
            self,
            text="C",
            command=lambda: self.buttonClicked(master, "C"),
            height=5,
            width=5
        )
        self.cButton.pack(side=LEFT)

        self.cSharpButton = Button(
            self,
            text="C#",
            command=lambda: self.buttonClicked(master, "C#"),
            height=5,
        ...
        self.dButton = Button(
            self,
            text="D",
            command=lambda: self.buttonClicked(master, "D"),
            height=5,
            width=5
        )
        self.dButton.pack(side=LEFT)

        self.eFlatButton = Button(
            self,
            text="Eb",
            command=lambda: self.buttonClicked(master, "Eb"),
            height=5,
            width=5
        )
        ...
        self.eButton = Button(
            self,
            text="E",
            command=lambda: self.buttonClicked(master, "E",),
            height=5,
            width=5)
        ...
        self.fButton = Button(
            self,
            text="F",
            command=lambda: self.buttonClicked(master, "F"),
            height=5,
            width=5
        )
        self.fButton.pack(side=LEFT)

        self.fSharpButton = Button(
            self,
            text="F#",
            command=lambda: self.buttonClicked(master, "F#"),
            height=5,
        ...
        self.gButton = Button(
            self,
            text="G",
            command=lambda: self.buttonClicked(master, "G"),
            height=5,
            width=5
        )
    ...
    def buttonClicked(self, master, selectedButton):
        ...
        # Get new image values
        newFileValues = master.selectRandomImage("sightReading")

        newImage = Image.open(
            f"images/sightReading/{newFileValues[0]}/{newFileValues[1]}/{newFileValues[2]}"
        )
        self.newImg = ImageTk.PhotoImage(newImage)
...
class TerminologyFlashCardsPage(ttk.Frame):
    def __init__(self, master):
        ...
        ttk.Label(
            self,
            text="Improve your knowledge of music\nterminology using flash cards.\n",
            font=("TkDefaultTheme", 32),
            justify=CENTER
        ).pack()
        ...
        self.flipButton = Button(
            self,
            text="Flip card",
            command=self.flipFlashCard,
            font=("TkDefaultTheme", 32)
        )
        self.flipButton.pack(side=LEFT)

        # Pick a new card button
        self.newCardButton = Button(
            self,
            text="New card",
            command=lambda: self.newFlashCard(master),
            font=("TkDefaultTheme", 32)
        )
        self.newCardButton.pack(side=RIGHT)

    def newFlashCard(self, master):
        newChosenTerm = randint(0, len(master.terminologyTerm) - 1)
...
class MusicSelectorPage(ttk.Frame):
    def __init__(self, master):
        ...
        for song in range(len(master.songPlayerDf)):
            Button(
                self,
                text=master.songNameList[song],
                command=lambda i=song: self.playSong(master, i),
                font=("TkDefaultTheme", 32),
                height=4
            ).pack()
...
class MusicPlayerPage(ttk.Frame):
    def __init__(self, master):
        ...
        ttk.Label(
            self,
            text=f" Title: {chosenSongTitle.strip()} ",
            font=("Helvetica", 20),
            justify=CENTER
        ).grid(row=0, column=6, columnspan=3)

        ttk.Label(
            self,
            text=f"Artist: {chosenSongArtist} ",
            font=("Helvetica", 20),
            justify=CENTER
        ...
        ttk.Label(self, text="\nMIXERS", font=("TkDefaultTheme", 28)).grid(row=7, columnspan=9)
...
class SettingsPage(ttk.Frame):
    def __init__(self, master):        
        ...
        self.darkModeCheckbox = ttk.Checkbutton(
            self,
            text="Dark Mode",
            command=lambda: master.changeTheme(master.appTheme)
        )
        ...
        self.playTestMusicCheckbox = ttk.Checkbutton(
            self,
            text="Play music in tests",
            command=lambda: master.changePlaySoundTestVariable(master.playTestSound)
        )
